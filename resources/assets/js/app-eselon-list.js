/**
 * Halaman Daftar Eselon
 * Dibuat dengan meniru pola app-eselon-list.js
 */
'use strict';

$(document).ready(function () {
  // --- UTILITIES (Sama Persis) ---
  var isSubmittingEselon = false;
  ...
  function initFormValidation(){
    setTimeout(function (){
      ...
      if (!eselonForm) {
        console.error('Form #eselonForm tidak ditemukan!');
        return;
      }
      ...
      try
      {
        console.log('Mencoba inisialisasi FV untuk #eselonForm...');
        console.log('FormValidation object:', typeof FormValidation, FormValidation);

        fv = FormValidation.formValidation(eselonForm, {
          fields: {
            nama_eselon: {
              validators: {
                notEmpty: { message: 'Nama Eselon wajib diisi.' },
                stringLength: { max: 50, message: 'Nama Eselon maksimal 50 karakter.' },
                regexp: {
                  regexp: /^[A-Za-z0-9\s\/\-\.]+$/,
  ...
  function initRealTimeValidation() {
    ...
    $('#eselonForm').on('input change blur', 'input, select, textarea', function () {
      var element = $(this);
  ...
  $('#eselonForm').on('show.bs.offcanvas', function () {
    console.log('Form opened, clearing all error messages');

    // Clear all error states
    $('#eselonForm').find('.is-invalid, .is-valid').removeClass('is-invalid is-valid');

    // Remove all error messages with animation
    var allErrorMessages = $('#eselonForm').find('.invalid-feedback');
  ...
  var table = $('.datatables-eselon').DataTable({
    processing: true,
    serverSide: true,
    ajax: '/eselon/data', // URL ke controller@data
    ...
    initComplete: function () {
      ...
      $('.dataTables_wrapper').on('change', 'select[name="datatables-eselon_length"]', function () {
        showDataTableLoading();
      });
    },
    ...
    language: {
      processing: 'Memuat data eselon...',
      loadingRecords: 'Memuat data eselon...',
      empty: 'Tidak ada data eselon',
    ...
    columns: [
      ...
      { data: 'nama_eselon', name: 'nama_eselon' }, // 2. Nama Eselon
    ...
    initComplete: function () {
      ...
      var addButton =
        '<button type="button" class="btn btn-primary" data-bs-toggle="offcanvas" data-bs-target="#offcanvasEselon">' +
        '<i class="ri-add-line me-sm-1"></i> <span class="d-none d-sm-inline-block">Tambah Eselon</span>' +
        '</button>';
    ...
    responsive: {
      details: {
        display: $.fn.dataTable.Responsive.display.modal({
          header: function (row) {
            var data = row.data();
            return 'Detail dari ' + data['nama_eselon'];
          }
        }),
        type: 'column',
  ...
  // 1. CREATE: Klik Tombol "Tambah Eselon"
  $('body').on('click', '.btn-primary[data-bs-target="#offcanvasEselon"]', function () {
    isSubmittingEselon = false;
    isAlertVisible = false;
    setButtonLoading(false);

    $('#eselonForm').trigger('reset'); // Reset form
    ...
    $('#eselonForm').find('.is-invalid, .is-valid').removeClass('is-invalid is-valid');
    $('#eselonForm').find('.invalid-feedback, .valid-feedback').remove(); // Hapus pesan error lama
    $('#eselon_id').val('');
    $('#offcanvasEselonLabel').html('Tambah Eselon');
  });
  ...
  $('body').on('click', '.edit-btn', function (e) {
    ...
    if (!id) {
      ...
      showAlertOnce({
        ...
        html: `<div style="text-align: left; max-width: 500px;">
          <div style="background: #fff5f5; padding: 15px; border-radius: 8px; border-left: 4px solid #dc3545; margin-bottom: 15px;">
            <p style="margin: 0; font-weight: 600; color: #dc3545; font-size: 1.1rem;">
              ID Eselon tidak ditemukan
            </p>
            <p style="margin: 5px 0 0 0; color: #6c757d; font-size: 0.95rem;">
              Tidak dapat menemukan ID Eselon yang diminta. Silakan refresh halaman dan coba lagi.
            </p>
          </div>
          <div style="background: #e3f2fd; padding: 10px; border-radius: 5px;">
            <p style="margin: 0; font-size: 0.9rem; color: #1976d2;">
    ...
    $.get('/eselon/' + id + '/edit', function (data) {
      $('#offcanvasEselonLabel').html('Edit Eselon');
      // Isi form
      $('#eselon_id').val(data.id_eselon);
      $('#nama_eselon').val(data.nama_eselon);
      // Tampilkan offcanvas
      $('#offcanvasEselon').offcanvas('show');
      ...
      showAlertOnce({
        ...
        html: `<div style="text-align: left; max-width: 500px;">
          <div style="background: #fff5f5; padding: 15px; border-radius: 8px; border-left: 4px solid #dc3545; margin-bottom: 15px;">
            ...
            <p style="margin: 5px 0 0 0; color: #6c757d; font-size: 0.95rem;">
              Terjadi masalah saat mengambil data Eselon dari server. Silakan coba lagi atau hubungi administrator jika masalah berlanjut.
            </p>
          </div>
          <div style="background: #e3f2fd; padding: 10px; border-radius: 5px;">
            <p style="margin: 0; font-size: 0.9rem; color: #1976d2;">
  ...
  setTimeout(function () {
    ...
    $('#eselonForm')
      ...
      .on('submit', function (e) {
        ...
        $('#eselonForm').find('.is-invalid, .is-valid').removeClass('is-invalid is-valid');
        $('#eselonForm').find('.invalid-feedback, .valid-feedback').remove();
        ...
        if (typeof fv === 'undefined' || fv === null) {
          ...
          var errorMessages = [];
          var requiredFields = [
            { name: 'nama_eselon', label: 'Nama Eselon' }
          ];

          requiredFields.forEach(function (field) {
            var element = $('#eselonForm').find('[name="' + field.name + '"]');
            console.log(
          ...
          if (isValid) {
            ...
            const firstError = $('#eselonForm').find('.is-invalid').first();
            if (firstError.length) {
              $('#offcanvasEselon .offcanvas-body').animate(
                {
                  scrollTop: firstError.offset().top - $('#offcanvasEselon .offcanvas-body').offset().top - 20
                },
                300
              );
            }
              ...
              if (status === 'Valid') {
                ...
                try {
                  ...
                  if (!hasInvalidFields || Object.keys(invalidFields).length === 0) {
                    ...
                    $('#eselonForm .is-invalid').removeClass('is-invalid');
                    $('#eselonForm .invalid-feedback').remove();
                  ...
                  for (const fieldName in invalidFields) {
                    ...
                    const element = $('#eselonForm').find(`[name="${fieldName}"]`);
                  ...
                  const errorField = $('#eselonForm').find('.is-invalid').first();
                  if (errorField.length) {
                    $('#offcanvasEselon .offcanvas-body').animate(
                      {
                        scrollTop: errorField.offset().top - $('#offcanvasEselon .offcanvas-body').offset().top - 20
                      },
                      300
                    );
                  }
  ...
  function submitFormAjax() {
    ...
    if (isSubmittingEselon) {
      console.warn('Submit is already in progress. Ignoring duplicate submit.');
      return;
    }
    isSubmittingEselon = true;

    var id = $('#eselon_id').val();
    var loadingMessage = id ? 'Memperbarui data...' : 'Menyimpan data eselon...';
    showLoadingScreen(loadingMessage);
    setButtonLoading(true);
    var formData = new FormData($('#eselonForm')[0]);
    var id = $('#eselon_id').val();
    var url = id ? '/eselon/' + id : '/eselon';
    ...
    if (csrfToken) {
      ...
      var formToken = $('#eselonForm').find('input[name="_token"]').val();
      if (formToken) {
    ...
    $.ajax({
      ...
      success: function (data) {
        ...
        setTimeout(function () {
          hideLoadingScreen();
          setButtonLoading(false);
          $('#offcanvasEselon').offcanvas('hide');
      ...
      error: function (xhr, status, error) {
        ...
        if (xhr.responseJSON && xhr.responseJSON.errors) {
          ...
          $.each(xhr.responseJSON.errors, function (key, value) {
            ...
            const element = $('#eselonForm').find(`[name="${key}"]`);
          ...
          if (firstError.length) {
            ...
            setTimeout(function () {
              const offcanvasBody = $('#offcanvasEselon .offcanvas-body');
  ...
  setTimeout(function () {
    ...
    $('body')
      .off('click', '#eselonForm button[type="submit"]')
      .on('click', '#eselonForm button[type="submit"]', function (e) {
        e.preventDefault();
        console.log('=== SUBMIT BUTTON CLICKED ===');
        console.log('Submit button clicked directly');
        console.log('Form element:', $('#eselonForm')[0]);
            ...
            error: function (data) {
              showAlertOnce({
                ...
                html: `<div style="text-align: left; max-width: 500px;">
                  <div style="background: #fff5f5; padding: 15px; border-radius: 8px; border-left: 4px solid #dc3545; margin-bottom: 15px;">
                    <p style="margin: 0; font-weight: 600; color: #dc3545; font-size: 1.1rem;">
                      Tidak dapat menghapus data eselon
                    </p>
                    <p style="margin: 5px 0 0 0; color: #6c757d; font-size: 0.95rem;">
