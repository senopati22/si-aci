 ...
 * Dibuat dengan meniru pola app-jabatan-list.js
 */
'use strict';

$(document).ready(function () {
  // --- UTILITIES (Sama Persis) ---
  var isSubmittingJabatan = false;
  ...
  function initFormValidation(){
    setTimeout(function (){
      ...
      if (!jabatanForm) {
        console.error('Form #jabatanForm tidak ditemukan!');
        return;
      }
      ...
      try
      {
        console.log('Mencoba inisialisasi FV untuk #jabatanForm...');
        console.log('FormValidation object:', typeof FormValidation, FormValidation);

        fv = FormValidation.formValidation(jabatanForm, {
          fields: {
            nama_jabatan: {
              validators: {
                notEmpty: { message: 'Nama Jabatan wajib diisi.' },
                stringLength: { max: 50, message: 'Nama Jabatan maksimal 50 karakter.' },
                regexp: {
                  regexp: /^[A-Za-z0-9\s\/\-\.]+$/,
  ...
  function initRealTimeValidation() {
    ...
    $('#jabatanForm').on('input change blur', 'input, select, textarea', function () {
      var element = $(this);
  ...
  $('#jabatanForm').on('show.bs.offcanvas', function () {
    console.log('Form opened, clearing all error messages');

    // Clear all error states
    $('#jabatanForm').find('.is-invalid, .is-valid').removeClass('is-invalid is-valid');

    // Remove all error messages with animation
    var allErrorMessages = $('#jabatanForm').find('.invalid-feedback');
  ...
  var table = $('.datatables-jabatan').DataTable({
    processing: true,
    serverSide: true,
    ajax: '/jabatan/data', // URL ke controller@data
    ...
    initComplete: function () {
      ...
      $('.dataTables_wrapper').on('change', 'select[name="datatables-jabatan_length"]', function () {
        showDataTableLoading();
      });
    },
    ...
    language: {
      processing: 'Memuat data jabatan...',
      loadingRecords: 'Memuat data jabatan...',
      empty: 'Tidak ada data jabatan',
    ...
    columns: [
      ...
      { data: 'nama_jabatan', name: 'nama_jabatan' }, // 2. Nama Jabatan
    ...
    initComplete: function () {
      ...
      var addButton =
        '<button type="button" class="btn btn-primary" data-bs-toggle="offcanvas" data-bs-target="#offcanvasJabatan">' +
        '<i class="ri-add-line me-sm-1"></i> <span class="d-none d-sm-inline-block">Tambah Jabatan</span>' +
        '</button>';
    ...
    responsive: {
      details: {
        display: $.fn.dataTable.Responsive.display.modal({
          header: function (row) {
            var data = row.data();
            return 'Detail dari ' + data['nama_jabatan'];
          }
        }),
        type: 'column',
  ...
  // 1. CREATE: Klik Tombol "Tambah Jabatan"
  $('body').on('click', '.btn-primary[data-bs-target="#offcanvasJabatan"]', function () {
    isSubmittingJabatan = false;
    isAlertVisible = false;
    setButtonLoading(false);

    $('#jabatanForm').trigger('reset'); // Reset form
    ...
    $('#jabatanForm').find('.is-invalid, .is-valid').removeClass('is-invalid is-valid');
    $('#jabatanForm').find('.invalid-feedback, .valid-feedback').remove(); // Hapus pesan error lama
    $('#jabatan_id').val('');
    $('#offcanvasJabatanLabel').html('Tambah Jabatan');
  });
  ...
  $('body').on('click', '.edit-btn', function (e) {
      ...
      showAlertOnce({
        ...
        html: `<div style="text-align: left; max-width: 500px;">
          <div style="background: #fff5f5; padding: 15px; border-radius: 8px; border-left: 4px solid #dc3545; margin-bottom: 15px;">
            ...
            <p style="margin: 5px 0 0 0; color: #6c757d; font-size: 0.95rem;">
              Tidak dapat menemukan ID Jabatan yang diminta. Silakan refresh halaman dan coba lagi.
            </p>
          </div>
          <div style="background: #e3f2fd; padding: 10px; border-radius: 5px;">
            <p style="margin: 0; font-size: 0.9rem; color: #1976d2;">
    ...
    $.get('/jabatan/' + id + '/edit', function (data) {
      $('#offcanvasJabatanLabel').html('Edit Jabatan');
      // Isi form
      $('#jabatan_id').val(data.id_jabatan);
      $('#nama_jabatan').val(data.nama_jabatan);
      // Tampilkan offcanvas
      $('#offcanvasJabatan').offcanvas('show');
      ...
      showAlertOnce({
        ...
        html: `<div style="text-align: left; max-width: 500px;">
          <div style="background: #fff5f5; padding: 15px; border-radius: 8px; border-left: 4px solid #dc3545; margin-bottom: 15px;">
            ...
            <p style="margin: 5px 0 0 0; color: #6c757d; font-size: 0.95rem;">
              Terjadi masalah saat mengambil data Jabatan dari server. Silakan coba lagi atau hubungi administrator jika masalah berlanjut.
            </p>
          </div>
          <div style="background: #e3f2fd; padding: 10px; border-radius: 5px;">
            <p style="margin: 0; font-size: 0.9rem; color: #1976d2;">
  ...
  setTimeout(function () {
    ...
    $('#jabatanForm')
      ...
      .on('submit', function (e) {
        ...
        $('#jabatanForm').find('.is-invalid, .is-valid').removeClass('is-invalid is-valid');
        $('#jabatanForm').find('.invalid-feedback, .valid-feedback').remove();
        ...
        if (typeof fv === 'undefined' || fv === null) {
          ...
          var errorMessages = [];
          var requiredFields = [
            { name: 'nama_jabatan', label: 'Nama Jabatan' }
          ];

          requiredFields.forEach(function (field) {
            var element = $('#jabatanForm').find('[name="' + field.name + '"]');
            console.log(
          ...
          if (isValid) {
            ...
            const firstError = $('#jabatanForm').find('.is-invalid').first();
            if (firstError.length) {
              $('#offcanvasJabatan .offcanvas-body').animate(
                {
                  scrollTop: firstError.offset().top - $('#offcanvasJabatan .offcanvas-body').offset().top - 20
                },
                300
              );
            }
              ...
              if (status === 'Valid') {
                ...
                try {
                  ...
                  if (!hasInvalidFields || Object.keys(invalidFields).length === 0) {
                    ...
                    $('#jabatanForm .is-invalid').removeClass('is-invalid');
                    $('#jabatanForm .invalid-feedback').remove();
                  ...
                  for (const fieldName in invalidFields) {
                    ...
                    const element = $('#jabatanForm').find(`[name="${fieldName}"]`);
                  ...
                  const errorField = $('#jabatanForm').find('.is-invalid').first();
                  if (errorField.length) {
                    $('#offcanvasJabatan .offcanvas-body').animate(
                      {
                        scrollTop: errorField.offset().top - $('#offcanvasJabatan .offcanvas-body').offset().top - 20
                      },
                      300
                    );
                  }
  ...
  function submitFormAjax() {
    ...
    var loadingMessage = id ? 'Memperbarui data...' : 'Menyimpan data jabatan...';
    ...
    if (csrfToken) {
      ...
      var formToken = $('#jabatanForm').find('input[name="_token"]').val();
      if (formToken) {
    ...
    $.ajax({
      ...
      success: function (data) {
        ...
        setTimeout(function () {
          hideLoadingScreen();
          setButtonLoading(false);
          $('#offcanvasJabatan').offcanvas('hide');
      ...
      error: function (xhr, status, error) {
        ...
        if (xhr.responseJSON && xhr.responseJSON.errors) {
          ...
          $.each(xhr.responseJSON.errors, function (key, value) {
            ...
            const element = $('#jabatanForm').find(`[name="${key}"]`);
          ...
          if (firstError.length) {
            ...
            setTimeout(function () {
              const offcanvasBody = $('#offcanvasJabatan .offcanvas-body');
  ...
  setTimeout(function () {
    ...
    $('body')
      .off('click', '#jabatanForm button[type="submit"]')
      .on('click', '#jabatanForm button[type="submit"]', function (e) {
        e.preventDefault();
        console.log('=== SUBMIT BUTTON CLICKED ===');
        console.log('Submit button clicked directly');
        console.log('Form element:', $('#jabatanForm')[0]);
            ...
            error: function (data) {
              showAlertOnce({
                ...
                html: `<div style="text-align: left; max-width: 500px;">
                  <div style="background: #fff5f5; padding: 15px; border-radius: 8px; border-left: 4px solid #dc3545; margin-bottom: 15px;">
                    <p style="margin: 0; font-weight: 600; color: #dc3545; font-size: 1.1rem;">
                      Tidak dapat menghapus data jabatan
                    </p>
                    <p style="margin: 5px 0 0 0; color: #6c757d; font-size: 0.95rem;">
