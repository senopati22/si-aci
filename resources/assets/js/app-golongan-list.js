/**
 * Halaman Daftar Golongan
 * Dibuat dengan meniru pola app-golongan-list.js
 */
'use strict';

$(document).ready(function () {
  // --- UTILITIES (Sama Persis) ---
  var isSubmittingGolongan = false;
  ...
  function initFormValidation(){
    setTimeout(function (){
      const golonganForm = document.getElementById('golonganForm');
      if (!golonganForm) {
        console.error('Form #golonganForm tidak ditemukan!');
        return;
      }
      ...
      try
      {
        console.log('Mencoba inisialisasi FV untuk #golonganForm...');
        console.log('FormValidation object:', typeof FormValidation, FormValidation);

        fv = FormValidation.formValidation(golonganForm, {
          fields: {
            nama_golongan: {
              validators: {
                notEmpty: { message: 'Nama Golongan wajib diisi.' },
                stringLength: { max: 50, message: 'Nama Golongan maksimal 50 karakter.' },
                regexp: {
                  regexp: /^[A-Za-z0-9\s\/\-\.]+$/,
  ...
  function initRealTimeValidation() {
    ...
    $('#golonganForm').on('input change blur', 'input, select, textarea', function () {
      var element = $(this);
  ...
  $('#golonganForm').on('show.bs.offcanvas', function () {
    console.log('Form opened, clearing all error messages');

    // Clear all error states
    $('#golonganForm').find('.is-invalid, .is-valid').removeClass('is-invalid is-valid');

    // Remove all error messages with animation
    var allErrorMessages = $('#golonganForm').find('.invalid-feedback');
  ...
  var table = $('.datatables-golongan').DataTable({
    processing: true,
    serverSide: true,
    ajax: '/golongan/data', // URL ke controller@data
    ...
    initComplete: function () {
      ...
      $('.dataTables_wrapper').on('change', 'select[name="datatables-golongan_length"]', function () {
        showDataTableLoading();
      });
    },
    ...
    language: {
      processing: 'Memuat data golongan...',
      loadingRecords: 'Memuat data golongan...',
      empty: 'Tidak ada data golongan',
    ...
    columns: [
      ...
      { data: 'nama_golongan', name: 'nama_golongan' }, // 2. Nama Golongan
    ...
    initComplete: function () {
      ...
      var addButton =
        '<button type="button" class="btn btn-primary" data-bs-toggle="offcanvas" data-bs-target="#offcanvasGolongan">' +
        '<i class="ri-add-line me-sm-1"></i> <span class="d-none d-sm-inline-block">Tambah Golongan</span>' +
        '</button>';
    ...
    responsive: {
      details: {
        display: $.fn.dataTable.Responsive.display.modal({
          header: function (row) {
            var data = row.data();
            return 'Detail dari ' + data['nama_golongan'];
          }
        }),
        type: 'column',
  ...
  // 1. CREATE: Klik Tombol "Tambah Golongan"
  $('body').on('click', '.btn-primary[data-bs-target="#offcanvasGolongan"]', function () {
    isSubmittingGolongan = false;
    isAlertVisible = false;
    setButtonLoading(false);

    $('#golonganForm').trigger('reset'); // Reset form
    ...
    $('#golonganForm').find('.is-invalid, .is-valid').removeClass('is-invalid is-valid');
    $('#golonganForm').find('.invalid-feedback, .valid-feedback').remove(); // Hapus pesan error lama
    $('#golongan_id').val('');
    $('#offcanvasGolonganLabel').html('Tambah Golongan');
  });
  ...
  $('body').on('click', '.edit-btn', function (e) {
    ...
    if (!id) {
      ...
      showAlertOnce({
        ...
        html: `<div style="text-align: left; max-width: 500px;">
          <div style="background: #fff5f5; padding: 15px; border-radius: 8px; border-left: 4px solid #dc3545; margin-bottom: 15px;">
            <p style="margin: 0; font-weight: 600; color: #dc3545; font-size: 1.1rem;">
              ID Golongan tidak ditemukan
            </p>
            <p style="margin: 5px 0 0 0; color: #6c757d; font-size: 0.95rem;">
              Tidak dapat menemukan ID Golongan yang diminta. Silakan refresh halaman dan coba lagi.
            </p>
          </div>
          <div style="background: #e3f2fd; padding: 10px; border-radius: 5px;">
            <p style="margin: 0; font-size: 0.9rem; color: #1976d2;">
    ...
    $.get('/golongan/' + id + '/edit', function (data) {
      $('#offcanvasGolonganLabel').html('Edit Golongan');
      // Isi form
      $('#golongan_id').val(data.id_golongan);
      $('#nama_golongan').val(data.nama_golongan);
      // Tampilkan offcanvas
      $('#offcanvasGolongan').offcanvas('show');
      ...
      showAlertOnce({
        ...
        html: `<div style="text-align: left; max-width: 500px;">
          <div style="background: #fff5f5; padding: 15px; border-radius: 8px; border-left: 4px solid #dc3545; margin-bottom: 15px;">
            ...
            <p style="margin: 5px 0 0 0; color: #6c757d; font-size: 0.95rem;">
              Terjadi masalah saat mengambil data Golongan dari server. Silakan coba lagi atau hubungi administrator jika masalah berlanjut.
            </p>
          </div>
          <div style="background: #e3f2fd; padding: 10px; border-radius: 5px;">
            <p style="margin: 0; font-size: 0.9rem; color: #1976d2;">
  ...
  setTimeout(function () {
    ...
    $('#golonganForm')
      ...
      .on('submit', function (e) {
        ...
        $('#golonganForm').find('.is-invalid, .is-valid').removeClass('is-invalid is-valid');
        $('#golonganForm').find('.invalid-feedback, .valid-feedback').remove();
        ...
        if (typeof fv === 'undefined' || fv === null) {
          ...
          var errorMessages = [];
          var requiredFields = [
            { name: 'nama_golongan', label: 'Nama Golongan' }
          ];

          requiredFields.forEach(function (field) {
            var element = $('#golonganForm').find('[name="' + field.name + '"]');
            console.log(
          ...
          if (isValid) {
            ...
            const firstError = $('#golonganForm').find('.is-invalid').first();
            if (firstError.length) {
              $('#offcanvasGolongan .offcanvas-body').animate(
                {
                  scrollTop: firstError.offset().top - $('#offcanvasGolongan .offcanvas-body').offset().top - 20
                },
                300
              );
            }
              ...
              if (status === 'Valid') {
                ...
                try {
                  ...
                  if (!hasInvalidFields || Object.keys(invalidFields).length === 0) {
                    ...
                    $('#golonganForm .is-invalid').removeClass('is-invalid');
                    $('#golonganForm .invalid-feedback').remove();
                  ...
                  for (const fieldName in invalidFields) {
                    ...
                    const element = $('#golonganForm').find(`[name="${fieldName}"]`);
                  ...
                  const errorField = $('#golonganForm').find('.is-invalid').first();
                  if (errorField.length) {
                    $('#offcanvasGolongan .offcanvas-body').animate(
                      {
                        scrollTop: errorField.offset().top - $('#offcanvasGolongan .offcanvas-body').offset().top - 20
                      },
                      300
                    );
                  }
  ...
  function submitFormAjax() {
    ...
    if (isSubmittingGolongan) {
      console.warn('Submit is already in progress. Ignoring duplicate submit.');
      return;
    }
    isSubmittingGolongan = true;

    var id = $('#golongan_id').val();
    var loadingMessage = id ? 'Memperbarui data...' : 'Menyimpan data golongan...';
    showLoadingScreen(loadingMessage);
    setButtonLoading(true);
    var formData = new FormData($('#golonganForm')[0]);
    var id = $('#golongan_id').val();
    var url = id ? '/golongan/' + id : '/golongan';
    ...
    if (csrfToken) {
      ...
      var formToken = $('#golonganForm').find('input[name="_token"]').val();
      if (formToken) {
    ...
    $.ajax({
      ...
      success: function (data) {
        ...
        setTimeout(function () {
          hideLoadingScreen();
          setButtonLoading(false);
          $('#offcanvasGolongan').offcanvas('hide');
      ...
      error: function (xhr, status, error) {
        ...
        if (xhr.responseJSON && xhr.responseJSON.errors) {
          ...
          $.each(xhr.responseJSON.errors, function (key, value) {
            ...
            const element = $('#golonganForm').find(`[name="${key}"]`);
          ...
          if (firstError.length) {
            ...
            setTimeout(function () {
              const offcanvasBody = $('#offcanvasGolongan .offcanvas-body');
  ...
  setTimeout(function () {
    ...
    $('body')
      .off('click', '#golonganForm button[type="submit"]')
      .on('click', '#golonganForm button[type="submit"]', function (e) {
        e.preventDefault();
        console.log('=== SUBMIT BUTTON CLICKED ===');
        console.log('Submit button clicked directly');
        console.log('Form element:', $('#golonganForm')[0]);
            ...
            error: function (data) {
              showAlertOnce({
                ...
                html: `<div style="text-align: left; max-width: 500px;">
                  <div style="background: #fff5f5; padding: 15px; border-radius: 8px; border-left: 4px solid #dc3545; margin-bottom: 15px;">
                    <p style="margin: 0; font-weight: 600; color: #dc3545; font-size: 1.1rem;">
                      Tidak dapat menghapus data golongan
                    </p>
                    <p style="margin: 5px 0 0 0; color: #6c757d; font-size: 0.95rem;">
